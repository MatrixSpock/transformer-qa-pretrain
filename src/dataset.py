...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        masked_start = random.randint(0, doc_len - masked_len)
        
        prefix = document[:masked_start]
        masked_content = document[masked_start:masked_start + masked_len]
        suffix = document[masked_start + masked_len:]
        ...
        if padding_needed > 0:
            masked_string = masked_string + self.PAD_CHAR * padding_needed
        
        # 4. Construct input and output strings
        x = masked_string[:-1]
        y = masked_string[1:]
